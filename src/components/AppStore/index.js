...
class AppStore extends Component {
  state = {activeTab: 'SOCIAL', userInput: ''}
  ...
  getFilteredData = () => {
    ...
    const filteredList = appsList.filter(
      eachOne => eachOne.category === activeTab,
    )
    return filteredList
  }

  changeOfUserInput = event => {
  ...
  getUserBasedList = () => {
    ...
    const searchedList = appsList.map(eachOne =>
      eachOne.appName.toLowerCase().includes(userInput),
    )
    return searchedList
  }
  ...
  render() {
    ...
    const searchedList = this.getUserBasedList()
    return (
      <div className="app-store-container">
        ...
        <div className="input-container">
          <input
            ...
            placeholder="search"
            onChange={this.changeOfUserInput}
          />
          <img
            src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"
            alt="search icon"
            className="search-icon-sizing"
          />
        </div>
        <ul className="unOrdered-list-container">
          {tabsList.map(eachItem => (
            <TabItem
              tabDetails={eachItem}
              key={eachItem.tabId}
              tabChanged={this.tabIsClicked}
            />
          ))}
        </ul>
        <hr />
        <ul className="unOrdered-list-app-container">
